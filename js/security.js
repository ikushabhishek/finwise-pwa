...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible" && localStorage.getItem('finwise-pin')) {
     if (document.getElementById('security-lock-screen').style.display === 'none') initSecurityEngine();
  }
});
...
function unlockAppSuccess() {
    ...
    currentPinInput = ""; updatePinDisplay();
}

function handlePinInput(num) {
  if (currentPinInput.length < 4) {
    currentPinInput += num; updatePinDisplay();
    if (currentPinInput.length === 4) setTimeout(processPinComplete, 200);
  }
}

function handlePinBackspace() {
  if (currentPinInput.length > 0) { currentPinInput = currentPinInput.slice(0, -1); updatePinDisplay(); }
}
...
function processPinComplete() {
  const savedPin = localStorage.getItem('finwise-pin');
  if (isSettingUpPin) {
    if (setupPinStep === 1) { tempSetupPin = currentPinInput; currentPinInput = ""; setupPinStep = 2; updatePinDisplay(); } 
    else {
      if (currentPinInput === tempSetupPin) {
        localStorage.setItem('finwise-pin', currentPinInput);
        document.getElementById('security-lock-screen').style.display = 'none';
        isSettingUpPin = false;
      } else { tempSetupPin = ""; currentPinInput = ""; updatePinDisplay(); }
    }
  } else if (isVerifyingToDisablePin) {
    if (currentPinInput === savedPin) unlockAppSuccess();
    else { currentPinInput = ""; updatePinDisplay(); }
  } else {
    if (currentPinInput === savedPin) unlockAppSuccess();
    else { currentPinInput = ""; updatePinDisplay(); }
  }
...
function togglePinSetupMode(checkbox) {
  ...
  if (!checkbox.checked && savedPin) {
    checkbox.checked = true; isVerifyingToDisablePin = true; currentPinInput = ""; updatePinDisplay();
    document.getElementById('security-lock-screen').style.display = 'flex';
    if(localStorage.getItem('finwise-biometric-id')) setTimeout(() => unlockWithBiometrics(true), 300);
    closeModal('preferences-modal'); return;
  }
  if (checkbox.checked) {
    closeModal('preferences-modal'); isSettingUpPin = true; setupPinStep = 1; tempSetupPin = ""; currentPinInput = ""; updatePinDisplay();
    document.getElementById('security-lock-screen').style.display = 'flex';
  }
}

function cancelPinSetup() {
    isSettingUpPin = false; isVerifyingToDisablePin = false;
    document.getElementById('security-lock-screen').style.display = 'none';
}
  ...
  try {
    ...
    if (!savedBioId) {
        const credential = await navigator.credentials.create({
            publicKey: { challenge, rp: { name: "FinWise", id: window.location.hostname }, user: { id: new Uint8Array(16), name: "user", displayName: "User" }, pubKeyCredParams: [{ type: "public-key", alg: -7 }], authenticatorSelection: { authenticatorAttachment: "platform", userVerification: "required" }, timeout: 60000 }
        });
        if (credential) {
        ...
        const assertion = await navigator.credentials.get({
            publicKey: { challenge, rpId: window.location.hostname, allowCredentials: [{ type: "public-key", id: rawId }], userVerification: "required" }
        });
